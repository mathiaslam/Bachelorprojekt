...
void printWord() {   
  if (refresh.isFinished()) {      
    ...
    for (int i=0; i<synonyms.size();i++) {
      ...
      foundSynonym = synonyms.get(i);
      int synonymWordLength = foundSynonym.length();

      msql.query("SELECT word FROM `blacklist` WHERE word = '"+foundSynonym+"' AND hidden_until > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 HOUR)");
      ...
      if (msql.next()) {
        ...
        myPort.write(foundSynonym+", ");
        linelength += foundSynonym.length()+2;
          myPort.write(TAB);
          myPort.write(TAB);
          myPort.write(TAB);
          myPort.write(TAB);
        for (int w=0; w < synonymWordLength; w++) {         
          myPort.write(TAB);
          myPort.write(TAB);
          myPort.write("#");
          myPort.write(TAB);
        ...
        for (int x=0; x < synonymWordLength; x++){
         myPort.write(" "); 
        }
